...
static int devnull_fd(void) {
  ...
  if (fd < 0) {
    fprintf(stderr, "Error opening /dev/null: %s", strerror(errno));
    return -1;
  }
...
static int tmpfile_fd(void) {
  ...
  if (fd < 0) {
    fprintf(stderr, "Error opening %s: %s", tmpfile_path, strerror(errno));
    return -1;
  }
...
START_TEST (ctrls_alloc_free_test) {
  ...
  mark_point();
  ctrl = pr_ctrls_alloc();
  next_ctrl = pr_ctrls_alloc();
  fail_unless(next_ctrl != NULL, "Failed to allocate next_ctrl: %s",
    strerror(errno));
  res = pr_ctrls_free(ctrl);
  fail_unless(res == 0, "Failed to free ctrl: %s", strerror(errno));
  res = pr_ctrls_free(next_ctrl);
  fail_unless(res == 0, "Failed to free next_ctrl: %s", strerror(errno));
}
END_TEST
...
START_TEST (ctrls_add_response_test) {
  ...
  mark_point();
  res = pr_ctrls_add_response(NULL, NULL, 0);
  fail_unless(res < 0, "Failed to handle null ctrl");
  fail_unless(errno == EINVAL, "Expected EINVAL (%d), got %s (%d)", EINVAL,
    strerror(errno), errno);
  ...
  mark_point();
  ctrl = pr_ctrls_alloc();
  res = pr_ctrls_add_response(ctrl, NULL, 0);
...
START_TEST (ctrls_recv_response_test) {
  ...
  rewind_fd(fd);
  res = pr_ctrls_recv_response(p, fd, &status, &respargv);
  fail_unless(res == respargc, "Failed to receive response: %s",
    strerror(errno));
...
START_TEST (ctrls_get_registered_actions_test) {
  ...
  fail_unless(res == 1, "Failed to handle GET_ACTION_ALL flag: %s",
    strerror(errno));

  mark_point();
  res = pr_get_registered_actions(ctrl, CTRLS_GET_ACTION_ENABLED);
  fail_unless(res == 1, "Failed to handle GET_ACTION_ENABLED flag: %s",
    strerror(errno));

  mark_point();
  res = pr_get_registered_actions(ctrl, CTRLS_GET_DESC);
  fail_unless(res == 1, "Failed to handle GET_DESC flag: %s", strerror(errno));
...
START_TEST (ctrls_run_ctrls_test) {
  ...
  /* cl_flags != PR_CTRLS_CL_HAVEREQ */
  /* cl_flags & PR_CTRLS_ACT_DISABLED */
  /* cl_flags !& PR_CTRLS_REQUESTED */
  /* mod != NULL && ctrl->ctrls_module != mod */
...
START_TEST (ctrls_connect_test) {
  ...
  /* XXX TODO More tests to add here! */
}
END_TEST
